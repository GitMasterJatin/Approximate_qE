import pandas as pd
import numpy as np
from datasketches import kll_floats_sketch, hll_sketch, hll_union
...
def calculate_accuracy(approx, exact):
    ...
    try:
        if isinstance(exact, dict):
            ...
            count = 0
            if not exact: 
                return ">>> Accuracy: N/A (empty exact result)"
            ...
            if count == 0: 
                return ">>> Accuracy: N/A (no valid groups to compare)"
            
            avg_relative_error = total_relative_error / count
            return f">>> Accuracy: (Avg. Relative Error: {avg_relative_error * 100:.4f}%)"
        
        elif isinstance(exact, (int, float)):
            # --- Logic for single numbers (COUNT DISTINCT, MEDIAN, etc.) ---
            if exact == 0:
                if approx == 0:
                    return ">>> Accuracy: 100.0000% (Error: 0.0000%)"
                return f">>> Accuracy: N/A (Exact result is 0)"
                
            error_frac = abs(approx - exact) / abs(exact)
            accuracy_pct = 100.0 * (1.0 - error_frac)
            return f">>> Accuracy: {accuracy_pct:.4f}% (Error: {error_frac * 100:.4f}%)"
            
    except Exception as e:
        return f">>> Accuracy: (Calculation Failed: {e})"
    
    return ">>> Accuracy: (Unsupported data type for comparison)"
        ...
        try:
            if q_type == 'COUNT' and len(parts) > 1 and parts[1] == 'DISTINCT':
                ...
                explanation = (
                    ...
                    f"This avoids a full scan of 5M rows."
                )
                ...
                explanation = (
                    ...
                    f"This allows for instant calculation of any quantile without sorting 5M rows."
                )
            ...
            return {"error": str(e), "explanation": explanation} # <-- ADDED EXPLANATION
...
# --- Main execution block (NOW PRINTS EXPLANATIONS) ---
if __name__ == "__main__":
    ...
    try:
        ...
        print("Please run 'python generate_data.py' (or 'loader/data.py') first to create it.")
        exit()
        
    print("Raw data loaded.")
    print("-" * 40)
    
    # --- 2. OFFLINE STEP: Fit the Engine ---
    aqe = FastAQE(sample_fraction=0.01, hll_precision=14)
    ...
    print(f"TOTAL OFFLINE INGESTION TIME: {end_fit - start_fit:.4f} sec")
    print("-" * 40)


    # --- 3. ONLINE STEP: Run Queries and Compare ---
    
    print("\n=== QUERY 1: COUNT DISTINCT ===")
    q1 = "COUNT DISTINCT user_id"
    
    approx_res_1 = aqe.query(q1)
    exact_res_1 = aqe.exact_query(raw_df, q1)
    
    # Print Explanation (prints for success or failure)
    print(f"[EXPLANATION]\n{approx_res_1['explanation']}\n")
    
    if 'error' in approx_res_1:
        print(f"[APPROXIMATE] Query Failed.")
    else:
        print(f"[APPROXIMATE] Result: {approx_res_1['approx_result']:.0f} (Time: {approx_res_1['query_time_sec']:.8f} sec)")
    
    print(f"[EXACT]       Result: {exact_res_1['exact_result']} (Time: {exact_res_1['query_time_sec']:.8f} sec)")
    
    if 'error' not in approx_res_1:
        print(calculate_accuracy(approx_res_1['approx_result'], exact_res_1['exact_result']))
        speedup_1 = exact_res_1['query_time_sec'] / approx_res_1['query_time_sec']
        print(f">>> Speedup: {speedup_1:.0f}x FASTER")
    print("-" * 40)

    print("\n=== QUERY 2: GROUP BY SUM ===")
    q2 = "SUM amount GROUP BY category"

    approx_res_2 = aqe.query(q2)
    exact_res_2 = aqe.exact_query(raw_df, q2)

    # Print Explanation
    print(f"[EXPLANATION]\n{approx_res_2['explanation']}\n")

    if 'error' in approx_res_2:
         print(f"[APPROXIMATE] Query Failed.")
    else:
        print(f"[APPROXIMATE] Time: {approx_res_2['query_time_sec']:.8f} sec")

    print(f"[EXACT]       Time: {exact_res_2['query_time_sec']:.8f} sec")

    if 'error' not in approx_res_2:
        print(calculate_accuracy(approx_res_2['approx_result'], exact_res_2['exact_result']))
        speedup_2 = exact_res_2['query_time_sec'] / approx_res_2['query_time_sec']
        print(f"\n>>> Speedup: {speedup_2:.0f}x FASTER")
    print("-" * 40)

    print("\n=== QUERY 3: MEDIAN (Quantile) ===")
    q3 = "MEDIAN value"

    approx_res_3 = aqe.query(q3)
    exact_res_3 = aqe.exact_query(raw_df, q3)

    # Print Explanation
    print(f"[EXPLANATION]\n{approx_res_3['explanation']}\n")

    if 'error' in approx_res_3:
        print(f"[APPROXIMATE] Query Failed.")
    else:
        print(f"[APPROXIMATE] Result: {approx_res_3['approx_result']:.2f} (Time: {approx_res_3['query_time_sec']:.8f} sec)")

    print(f"[EXACT]       Result: {exact_res_3['exact_result']:.2f} (Time: {exact_res_3['query_time_sec']:.8f} sec)")
    
    if 'error' not in approx_res_3:
        print(calculate_accuracy(approx_res_3['approx_result'], exact_res_3['exact_result']))
        speedup_3 = exact_res_3['query_time_sec'] / approx_res_3['query_time_sec']
        print(f">>> Speedup: {speedup_3:.0f}x FASTER")
    print("-" * 40)
    
    print("\n=== QUERY 4: 95th Percentile (Quantile) ===")
    q4 = "QUANTILE value 0.95"
    
    approx_res_4 = aqe.query(q4)
    exact_res_4 = aqe.exact_query(raw_df, q4)

    # Print Explanation
    print(f"[EXPLANATION]\n{approx_res_4['explanation']}\n")

    if 'error' in approx_res_4:
        print(f"[APPROXIMATE] Query Failed.")
    else:
        print(f"[APPROXIMATE] Result: {approx_res_4['approx_result']:.2f} (Time: {approx_res_4['query_time_sec']:.8f} sec)")

    print(f"[EXACT]       Result: {exact_res_4['exact_result']:.2f} (Time: {exact_res_4['query_time_sec']:.8f} sec)")

    if 'error' not in approx_res_4:
        print(calculate_accuracy(approx_res_4['approx_result'], exact_res_4['exact_result']))
        speedup_4 = exact_res_4['query_time_sec'] / approx_res_4['query_time_sec']
        print(f">>> Speedup: {speedup_4:.0f}x FASTER")
    print("-" * 40)